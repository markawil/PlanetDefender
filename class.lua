...
function Class( base )
    ...
    seekVariable = function( self, value, ... )
        ...
        if value == 'instanceFunctions' or value == 'base' or not functionName then
    ...
    callMetamethod.__call = function( self, ... )
        -- Make an instance and add the __index metamethod to call its functions
        local instance = { }
        ...
        if waterfallConstructors == true then
            ...
            while tempBase ~= nil do
                if tempBase.new and tempBase.new ~= tempCurrent.new then
                    -- Call the base constructor if it exists
                    constructors[ #constructors + 1 ] = tempBase.new
